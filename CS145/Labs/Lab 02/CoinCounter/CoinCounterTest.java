...
public class CoinCounterTest extends CoinCounter { // BAD WRONG SO MUCH WRONG
	
	@SuppressWarnings("unlikely-arg-type")
	@Test
	public final void handlesSingleTypes() {
		Assert.assertEquals((long) CoinCounter.calculate(25).get(25),  1);
		Assert.assertEquals((long) CoinCounter.calculate(10).get(10), 1);
		Assert.assertEquals((long) CoinCounter.calculate(5).get(5), 1);
		Assert.assertEquals((long) CoinCounter.calculate(1).get(1), 1);
	}
